...
* 某个 acceptor 发生故障, 就不能出现多数派, 不能得到决议; 这时, 就可能需要进行多次提交提案, 进行多次批准.
...
* 然后又想出来 P2b, P2b 和 P1 不冲突, 只要做到 P1 和 P2b 都满足就可以解决问题; 但 P2b 比较难实现
...
同时, Paxos 规定, 在全局范围内 (所有的 processes), 每个提案有唯一的版本号, 这样所有的版本号就形成了一个全序关系.

而且, 既然所有提案的版本号是全局范围内的全序关系, 也就给所有的提案进行了逻辑时间上的排序:

两个提案: V{v, n}, W{w, m}, 如果 n > m, 那么**提案 V 比 提案 W 后提出**
...
前面已经说过, 版本号就是逻辑上的全局时间, m > n, 代表, {w, m} 是在 {v, n} 之后的提案.
...
P2a: 假设一个提案 {v, n} 已经被选为决议, 所有被 acceptor 批准的提案 {w, m}, 如果 m >n, 那么内容 w 必须和内容 v 相同
...
* 提案 V{v, n} 已经被选为决议
* 但在这次选举过程中, 某个 acceptor 没有批准 V{v, n} (也就是说, 这个 acceptor 还可以批准更高版本号的提案, 例如: W{w, m}, m > n)
* 之后, 又有一个 proposer 提交了一个提案 W{w, m} 给上面这个 acceptor, 而根据 P1, 这个 acceptor 会批准这个提案, 且这个提案的 m > n, w 和 v 不相同
* 这就和 P2a 矛盾了: 在 V{v, n} 已经被选为决议的情况下, 任何 acceptor 批准的更高版本 (> n) 的提案的内容必须和 v 相同

说明: 在有一个决议已经形成的情况下, 是有可能发生每个 proposer 提出新的不同内容的提案的可能的.
...
2. 存在一个 acceptors 的多数派 S, 集合中的某些 acceptor 曾经批准过小于 n 的提案, 而所有的这些提案中, 拥有最大的版本号那个提案的内容与 v 相同

也就是说, 如果每个 proposer 都按 P2c 为约束来提出一个提案, 那么就可以满足 P2b.
...
保证 P2c, 也就是要保证 proposer 在提出一个提案是, 必须满足 P2c. 所以 proposer 想要提交提案时需要 2 个步骤:

1. proposer 选择一个版本号 n, 然后向一个 acceptors 的集合发送请求, 要求 acceptor 做出以下动作:
 1. 保证不再批准任何小于 n 的提案
 2. 返回这个 acceptor 到目前为止已经通过的, 且小于 n 的最大版本号的提案. 返回消息为: 内容+版本号 (如果没有批准过任何版本号小于 n 提案, 返回一个空值)
2. 如果 proposer 收到了一个多数派的返回响应, 那么这个 proposer 可以提出版本号为 n 的提案, 其对应的内容为:
...
* acceptor 端收到 proposer 端的两种请求都可以不做任何回答 (因为不回答不会破坏算法的安全性)
* 如果要对 prepare 请求做出响应, 只要做出承诺, 并根据其当前的状态返回响应就行了 (找出目前批准过的最大版本号的提案作为响应返回)
